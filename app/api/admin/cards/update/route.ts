...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabaseAdmin.from('card_requests').update({ status }).eq('id', id).select().single()
    ...
    if (user?.email) {
      // Build email content based on approval status
      const subject = action === 'approve' 
        ? 'Your WilliamsHoldings Visa is approved' 
        : 'Your WilliamsHoldings Visa application'
      
      let body = action === 'approve' 
        ? `Hi, your card request (id: ${id}) was approved.` 
        : `Hi, your card request (id: ${id}) was not approved.`
